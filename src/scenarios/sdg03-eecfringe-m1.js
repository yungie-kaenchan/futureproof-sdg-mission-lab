/**
 * FUTUREPROOF — SDG 3 EEC fringe — "The Village the Boom Left Behind" Mission
 *
 * Third fully-built journey mission. Mirrors the proven Chiang Mai M1
 ...
 *
 *   2. KEYSTONE ON PASS — completing the mission awards the SDG-3
...
const MISSION_ID = "sdg03-eecfringe"; // must match keystones.js JOURNEY_MISSIONS id

/* Resolved once at mission start so every phase renders at the same tier. */
let READING_TIER = 2;

export function installEecFringeMission(engine) {
...
export const installEecFringeMission1 = installEecFringeMission;
  ...
  try {
    ...
    if (flow?.uid && isFirebaseAvailable() && finalAward !== 0) {
      await awardTokens({
        tid: flow.uid,
        delta: finalAward,
        reason: `eecfringe_quiz_${item.id}`,
  ...
  if (passed) {
    ...
    try {
      ...
      const res = await awardKeystone(flow && flow.uid, MISSION_ID, {
        source: "mission",
        reason: `The Village the Boom Left Behind passed (net ${totalTokens} tokens >= ${PASS_THRESHOLD_TOKENS}).`,
      });
      keystoneMsg = res.alreadyHad
        ? "You already held this Keystone — no double-count."
        : (res.earned ? "SDG 3 Keystone earned and recorded."
  ...
  paint(container, `
    <section class="mission-complete">
      <div class="complete-banner">
        ...
        <h2 class="display-heading text-3xl mt-2">${passed ? "The Village the Boom Left Behind — cleared." : "Close. One more pass."}</h2>
        <p class="body-l text-on-surface-variant mt-2">
          ${passed
      ...
      <div class="complete-stats">
        ...
        <div class="complete-stat">
          <div class="console-label-gold">SDG 3 KEYSTONE</div>
