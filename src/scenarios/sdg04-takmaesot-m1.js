/**
 * FUTUREPROOF — SDG 4 Mae Sot — "The Children at the Border" Mission
 *
 ...
 *
 *   2. KEYSTONE ON PASS — completing the mission awards the SDG-4
...
const MISSION_ID = "sdg04-takmaesot"; // must match keystones.js JOURNEY_MISSIONS id

/* Resolved once at mission start so every phase renders at the same tier. */
let READING_TIER = 2;

export function installMaeSotMission(engine) {
...
export const installMaeSotMission1 = installMaeSotMission;
  ...
  if (passed) {
    ...
    try {
      ...
      const res = await awardKeystone(flow && flow.uid, MISSION_ID, {
        source: "mission",
        reason: `The Children at the Border passed (net ${totalTokens} tokens >= ${PASS_THRESHOLD_TOKENS}).`,
      });
      keystoneMsg = res.alreadyHad
        ? "You already held this Keystone — no double-count."
        : (res.earned ? "SDG 4 Keystone earned and recorded."
  ...
  paint(container, `
    <section class="mission-complete">
      <div class="complete-banner">
        ...
        <h2 class="display-heading text-3xl mt-2">${passed ? "The Children at the Border — cleared." : "Close. One more pass."}</h2>
        <p class="body-l text-on-surface-variant mt-2">
          ${passed
      ...
      <div class="complete-stats">
        ...
        <div class="complete-stat">
          <div class="console-label-gold">SDG 4 KEYSTONE</div>
