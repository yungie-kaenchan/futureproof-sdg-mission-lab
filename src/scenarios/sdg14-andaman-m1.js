/**
 * FUTUREPROOF — SDG 14 Andaman — "The Reef and the Tide" Mission
 *
...
const MISSION_ID = "sdg14-andaman"; // must match keystones.js JOURNEY_MISSIONS id

/* Resolved once at mission start so every phase renders at the same tier. */
let READING_TIER = 2;

export function installAndamanMission(engine) {
...
export const installAndamanMission1 = installAndamanMission;
...
// Same shape + classes as Khon Kaen so the shared CSS picks it up.
  ...
  if (passed) {
    ...
    try {
      ...
      const res = await awardKeystone(flow && flow.uid, MISSION_ID, {
        source: "mission",
        reason: `The Reef and the Tide passed (net ${totalTokens} tokens >= ${PASS_THRESHOLD_TOKENS}).`,
      });
      keystoneMsg = res.alreadyHad
        ? "You already held this Keystone — no double-count."
        : (res.earned ? "SDG 14 Keystone earned and recorded."
  ...
  paint(container, `
    <section class="mission-complete">
      <div class="complete-banner">
        ...
        <h2 class="display-heading text-3xl mt-2">${passed ? "The Reef and the Tide — cleared." : "Close. One more pass."}</h2>
        <p class="body-l text-on-surface-variant mt-2">
          ${passed
      ...
      <div class="complete-stats">
        ...
        <div class="complete-stat">
          <div class="console-label-gold">SDG 14 KEYSTONE</div>
